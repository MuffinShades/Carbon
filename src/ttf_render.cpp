...
i32 ttfRender::RenderGlyphMSDFToBitMap(Glyph tGlyph, Bitmap* map, sdf_dim size) {
    ...
    const f32
        wc = glyphW / (f32) map->header.w,
        hc = glyphH / (f32) map->header.h;

    byte color;

    vec3 *distBuffer = new vec3[map->header.w * map->header.h]; //i hate this so much
    ...
    for (y = 0; y < map->header.h; ++y) {
        for (x = 0; x < map->header.w; ++x) {
            p.x = (((f32)x) + 0.5f) * wc + tGlyph.xMin;
            p.y = (((f32)y) + 0.5f) * hc + tGlyph.yMin;
            ...
            distBuffer[x + y * map->header.w] = vec3(dr.d,dg.d,db.d);
    ...
    for (y = 0; y < map->header.h; ++y) {
        for (x = 0; x < map->header.w; ++x) {
            ...
            const size_t mp = distIdx * nChannels;
